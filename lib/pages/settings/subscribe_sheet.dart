...
///
/// STUB: no real payment/billing backend exists anywhere in the app (see
/// flows/sdd-vpnclient-payment) — this is the UI shell only, with mocked
/// plan data. `_pay()` just flips to the success overlay after a delay.
...
// TODO(sdd-vpnclient-payment): Sber Pay / T-Bank Pay / ЮMoney / QR-код have
// no exported bitmap in the Figma file (their Property1* components are
// built from raw vector paths, not a packaged asset) — only Visa/
// Mastercard/Mir/СБП/crypto had a real Vector.svg to copy. To finish these,
// re-export each missing logo from Figma as SVG, drop it in
// assets/images/payment/, and add its path here.
const _ruMethods = [
...
class _SubscribeSheetState extends State<SubscribeSheet> {
  ...
  // TODO(sdd-vpnclient-payment): a real purchase must be sent to a payment
  // provider (per the chosen _paymentMethod) and, on success, the resulting
  // subscription (plan, expiry, autopay) should be persisted and reflected
  // in Settings' identity/plan row instead of the static placeholder there.
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            ...
            style: TextStyle(fontSize: 17, color: AppColors.textPrimaryLight),
          ),
                  ...
                  Row(
                    children: [
                      Text(
                        plan.label,
                        style: const TextStyle(
                            fontSize: 17, color: AppColors.textPrimaryLight),
                      ),
            ...
            children: [
              Expanded(
                child: Text(
                  ...
                  style: const TextStyle(fontSize: 17, color: AppColors.textPrimaryLight),
                ),
              ),
              Text(
                '$_price ₽',
      ...
      child: Column(
        ...
        children: [
          ...
          TextField(
            ...
            // TODO(sdd-vpnclient-payment): add real formatting/validation —
            // group digits in 4s as the user types, run a Luhn checksum, and
            // detect the card network (Visa/Mastercard/Mir prefixes) to swap
            // in the matching logo. Same for MM/YY (reject past dates) and
            // CVV (require exactly 3-4 digits).
            decoration: InputDecoration(
              filled: true,
              fillColor: AppColors.surfaceLight,
              ...
              child: Row(
                children: [
                  Icon(
                    ...
                    color: _autopay ? AppColors.brandBlue : AppColors.textMuted,
                  ),
                  const SizedBox(width: 8),
                  const Expanded(
                    child: Text(
